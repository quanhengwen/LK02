...
extern u16 AGC_NOW;
extern void Contrl_635LD(u8 state);
//-----------------------------------------------------------------------------
...
void CMD_ReadThd(int dat)
{
  //ThresholdBool=dat;
  if(SysSeting.Language == 1)
    pro_printf("开关门限为: %dcm\n",SysSeting.SWvalue);
  else
    pro_printf("Switch threshold is: %dcm\n",SysSeting.SWvalue);
