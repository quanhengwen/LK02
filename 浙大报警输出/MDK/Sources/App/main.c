...
//数据滤波，
//**确保滤波在不同的输出频率下计算耗时相等
enum  {normal,rise,lower} data_typ_enu;
extern uint16_t erro_dist_cnt ;  //错误数据计数  :zjk 
uint32_t diff=0;
int16_t z_lis_dist=0;
uint8_t z_index = 0,z_index_down=0;
#define Z_RANGE    1000
extern uint16_t erro_dist_cnt;
u32 FilterBublesort(u32 *p,u32 n)
 {
     u32 i,j,k,erro_cnt=0,dis;
      u64 Sum_Arry=0;
      if(!n)
        {return 0;}
	    for(int i=0;i<n-1-erro_dist_cnt;i++)
			{
				 //add			 
					z_lis_dist = *(p+i+1)-*(p+i); //
					if(z_lis_dist>Z_RANGE)   //从小到大
					{ 
						z_index = i; //下一个位置开始跳变
						data_typ_enu =  rise;
						break;
          ...
          if(z_lis_dist < 0)				//从大到小
					{
						 if(abs(z_lis_dist) >Z_RANGE)
						 {
					  	 z_index = i; //下一个位置开始跳变
							 z_index_down =i;
					     data_typ_enu =  lower;
	  ...
	  switch(data_typ_enu)
		{
		  case normal:
			    {
						 for(i=0;i<n-erro_dist_cnt;i++)//
						 {
							 Sum_Arry+=*(p+i);//求和
						 }						
						dis = Sum_Arry/(n-erro_dist_cnt);
            erro_dist_cnt =0; 								 
					}
				break;
		  case lower:
		  ...
		  case rise:
			    {
						 for(i=z_index+1;i<n-erro_dist_cnt;i++)//从远到近,或者从近到远,取后半数据
						 {
							 Sum_Arry+=*(p+i);//求和
						 }						
						 dis = Sum_Arry/(n-erro_dist_cnt-z_index);
						 erro_dist_cnt =0; 	
...
//脉冲测量函数
uint8_t zjk_erro=0,z_dist_f[200]={0};
  
void meas_range(void)
{
  u8  bool_out=0;
  int ret;
  u32 dist_tim,z_dist_f_cal=0;
  u8 txbuf[12]={0};
  u16 temp=0;
  u8 tmpbuf[20] = {0};
  u8 tmpsize = 0;
