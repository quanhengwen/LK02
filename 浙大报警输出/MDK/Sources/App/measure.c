...
void Mode_Config(u8 mode)
{
      if(mode==1)
      {
        ...
        gp22_write_config_reg(OP_CODE_WR(0x00), 0x00242712);
        gp22_write_config_reg(OP_CODE_WR(0x01), 0x01410023);//0x01420023
      ...
      gp22_write_config_reg(OP_CODE_WR(0x00), 0x00142f12);//0x00242f12--4fps，0x00142f12--2fps
      gp22_write_config_reg(OP_CODE_WR(0x01), 0x21420023);//0x21420023--mode2//0x01410000--mode1
...
// 校准测量在需要校准的时候校准***
//##################################################
extern u32 range_Get(u32 d);

uint16_t erro_dist_cnt =0;  //错误数据计数
int trigger_pulse(u32 *pDist)
{
    ...
    {
         //zjk add
			   dist =  gp22_read_dword(OP_CODE_RD(0x00));
			   if((dist<655360) &&(dist!=0))
				 {
				    *pDist=dist;
						z_dist =CalcDistance(dist);//根据时间解算距离，以及初步标定
						//标定数据处理**根据脉冲频率选择标定数据
						z_dist=range_Get(z_dist);
						if(z_dist<SysSeting.SWvalue)
						{
