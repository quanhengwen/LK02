...
void CMD2_SetFrameType(ModbusRequest *p_request, ModbusResponse *p_response)
{
   
  if(p_request->Dat[1] == 1){
    
      SysSeting.frameType = 1;
    //  p_response->Dat[0] = 1;
    
  }else{
    SysSeting.frameType = 0;
   // p_response->Dat[0] = 0;
  }
  SettingSave();
}
...
const TypeModbusHandle ModBusHandleTab[] = {
    ...
    {0x01, CMD2_SingMeas},    //单次测量
	  {0x02, CMD2_StartMeas},  //开始测量
    {0x03, CMD2_GetBase},     //获取基准
    {0x04, CMD2_GetBaudrate},  //获取波特率		
		{0x05, CMD2_GetAddress},     //获取从机地址
		{0x06, CMD2_GetAutorun},   //获取上电自动测量状态
//    {0x07, CMD2_GetSwitchthd},  //读取开关门限
//    {0x08, CMD2_GetSwitchResult}, //读取开关结果
    {0x09, CMD2_GetOutputHZ},	//读取输出频率
		
		
	 //  {0x07, CMD2_GetPulseFreq},  //脉冲重复频率	
	//{0x04, CMD2_GetVersion},     //获取系统软硬件版本号		
   // {0x0A, CMD2_GetCurrentRange},//电流环量程

    /*设置*/
    {0x10, CMD2_StopMeas},      //停止测量
    {0x11, CMD2_SetBase},        //设置基准
    {0x12, CMD2_SetBaudrate},     //设置波特率
    {0x13, CMD2_SetSwitchthd},  //设置开关门限
  	{0x14, CMD2_SetAutorun},      //设置开机自动运行		
	  {0x15, CMD2_SetAddress},      //设置地址   		
		
	  {0x16, CMD2_SetOutputHZ},	//设置输出频率
	  {0x1f, CMD2_SetFrameType},	//设置ASCII 或 modbus 输出
