...
void TimerBoard_Init()
{
  unsigned short tmv_set = 0;
    ...
    //SysSeting.CurMol = 1;
    GPIO_SetBits(GPIOB,GPIO_Pin_8);//rs485
	hr_TimerBaseInit(TIM3,63,999);   //定时器3周期1000ms
	HwTimer_InterrtpyInit();
    if(SysSeting.CurMol == 255){//new eerom sectors(without system params)
      SysSeting.CurMol = 0;
      DefaultSetting(); //默认参数设置
      SettingSave();    //存储flash
      sprintf(UsartDatStr.TxBuffer,"当前为标定模式，请标定! AA xx xx BB\r\n");
      ...
      //UsartDatStr.TxSize=strlen(UsartDatStr.TxBuffer);
      Printf("当前为标定模式，请标定!AA xx xx BB\r\n");
...
void mPacket(unsigned char* buf,unsigned short dist);
...
void TIM3_IRQHandler()
{
  ...
  if((MeasStatus||FreeRun)  && !Flag_Cal &&(MeasStatus == CONTINUE_MEAS)){
      if(UsartDatStr.FinishedTxFlag){
        if(OutputTimeStr.OutputTimeCnt == 0){
          OutputTimeStr.OutputTimeCnt = OutputTimeStr.SuposOutputHzCnt;
          if(SysSeting.CurMol != 0 && ProtocolType == PROTOCOL_ASCII){
//            GPIO_SetBits(GPIOB,GPIO_Pin_8);
//            //mPacket(UsartDatStr.TxBuffer,dist_final);

 /*           if(dist_final == 0){
              sprintf(UsartDatStr.TxBuffer,"---\r\n");
            }else{
              sprintf((char*)UsartDatStr.TxBuffer,"%.2fm\r\n",dist_final/100.0);
						 
            }
            //GPIO_WriteBit(GPIOB,GPIO_Pin_8,!GPIO_ReadInputDataBit(GPIOB,GPIO_Pin_8));
            UsartDatStr.TxSize = strlen((char *)UsartDatStr.TxBuffer);
            UsartDatStr.FinishedTxFlag = 0;
            Uart1_DMASendOutOnce((char *)UsartDatStr.TxBuffer,UsartDatStr.TxSize);*/
          }
        }
      }
    }
